...
import com.shashank.common.dto.ResponseBuilder;
...
public class BookApi {
	...
	public ResponseEntity<?> getHelloString() {
		try {
			// Sample service
	...
	public ResponseEntity<?> fetchAllBooks() {
		try {
			...
			return new ResponseEntity<>(ResponseBuilder.builder().status(1)
	...
	public ResponseEntity<?> saveBook(@Valid @RequestBody Book book) {
		try {
			...
			return new ResponseEntity<>(ResponseBuilder.builder().status(1).message(messageSource
